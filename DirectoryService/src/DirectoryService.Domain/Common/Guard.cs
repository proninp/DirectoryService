...
internal static class Guard
{
        ...
        int maxLength)
    {
        ...
        {
            return UnitResult.Failure<Errors>(GeneralErrors.ValueIsRequired(fieldName));
        }

        if (fieldValue.Length < minLength)
            return UnitResult.Failure<Errors>(GeneralErrors.InvalidFieldLength(fieldName, minLength: minLength));

        if (maxLength > 0 && fieldValue.Length > maxLength)
            return UnitResult.Failure<Errors>(GeneralErrors.InvalidFieldLength(fieldName, maxLength: maxLength));
    ...
    public static UnitResult<Errors> ValidateLatinString(string fieldValue, string fieldName)
    {
        return !LatinOnlyRegex.IsMatch(fieldValue)
            ? UnitResult.Failure<Errors>(
                GeneralErrors.ValueIsInvalid(fieldName, $"{fieldName} must contain only latin letters"))
